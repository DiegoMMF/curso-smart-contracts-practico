...
contract TicTacToe {
    ...
    function chequearLinea(uint[4][4] memory jugadas, uint x1,uint y1,uint x2,uint y2,uint x3,uint y3) private returns(uint) {
        if ((jugadas[x1][y1] == jugadas[x2][y2]) && (jugadas[x2][y2] == jugadas[x3][y3]))
            return jugadas[x1][y1];
        return 0;
    }

    function obtenerGanador(Partida memory partida) private returns(uint) {
        // Check diag \
